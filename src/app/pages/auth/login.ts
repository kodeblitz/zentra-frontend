                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="text-center mb-8">
                            ...
                            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Zentra</div>
                        ...
                        <form (ngSubmit)="onSubmit()">
                            ...
                            <input pInputText id="username1" type="text" placeholder="Usuario" class="w-full md:w-120 mb-8" [(ngModel)]="username" name="username" />

                            <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Contraseña</label>
                            <p-password id="password1" [(ngModel)]="password" placeholder="Contraseña" [toggleMask]="true" styleClass="mb-4" [fluid]="true" [feedback]="false" name="password"></p-password>

                            <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                                <div class="flex items-center">
                                    <p-checkbox [(ngModel)]="rememberMe" id="rememberme1" binary class="mr-2" name="rememberMe"></p-checkbox>
                                    <label for="rememberme1">Recordarme</label>
                                </div>
                            </div>
                            <p-button type="submit" label="Iniciar sesión" styleClass="w-full" [loading]="loading"></p-button>
                        </form>
                    </div>
                </div>
            </div>
...
export class Login {
    ...
    onSubmit(): void {
        ...
        if (!u || !p) {
            this.messageService.add({ severity: 'warn', summary: 'Campos requeridos', detail: 'Ingrese usuario y contraseña.' });
            return;
        }
        this.loading = true;
        this.authService.login(u, p).subscribe({
            ...
            error: (err) => {
                ...
                this.messageService.add({ severity: 'error', summary: 'Error al iniciar sesión', detail: msg });
            }
        });
    }
}
